...
2. Затем необходимо установить все зависимости. Для этого перейдите в директорию с проектом и введите следующие команды:
```bash
go mod download
go mod tidy
```

3. Следующим этапом идёт объявление переменных окружения (не обязательно, по умолчанию задержка у всех будет равна `100` ms, порт у оркестратора и агента - `8080` и `8081` соответственно, а максимальное кол-во горутин - `5`).
...
4. Затем можно производить запуск (первоначально нужно запустить оркестратор, затем уже агент (как микросервис)).
...
## ps
я уже понял, что всё это дело надо переписывать хотя бы с 0
